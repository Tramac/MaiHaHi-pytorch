...
def detect(net, img, device='cuda'):
...
def batch_detect(net, imgs, device="cuda"):
    ...
    for i in range(len(olist) // 2):
        ...
        for Iindex, hindex, windex in poss:
            ...
            priors = torch.Tensor([[axc / 1.0, ayc / 1.0, stride * 4 / 1.0, stride * 4 / 1.0]]).reshape([1, 1, 4])
            variances = [0.1, 0.2]
            box = batch_decode(loc, priors, variances)
            box = box[:, 0] * 1.0
            bboxlist.append(torch.cat([box, score.unsqueeze(1)], 1).numpy())
    bboxlist = np.array(bboxlist)
    if 0 == len(bboxlist):
